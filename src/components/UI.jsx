...
export function GlassCard({ children, className = '', hover = false }) {
  return (
    <div className={`relative rounded-2xl border border-white/[0.06] overflow-hidden ${hover ? 'hover:border-cyan-500/20 transition-all duration-300 hover:shadow-lg hover:shadow-cyan-500/5' : ''} ${className}`} style={{ background: 'linear-gradient(135deg, rgba(255,255,255,0.03) 0%, rgba(255,255,255,0.01) 100%)', backdropFilter: 'blur(20px)' }}>
    ...
    <GlassCard hover>
      <div className={`p-5 border-l-2 ${c.border}`}>
        <div className="flex items-start justify-between mb-3">
          ...
          {Icon && <div className={`w-8 h-8 rounded-lg bg-gradient-to-br ${c.bg} flex items-center justify-center`}><Icon size={16} className={c.text} /></div>}
        </div>
...
export function CustomTooltip({ active, payload, label, formatter }) {
  ...
  return (
    <div className="rounded-xl border border-white/10 px-4 py-3" style={{ background: 'rgba(10,10,30,0.95)', backdropFilter: 'blur(10px)' }}>
      <p className="text-xs text-white/50 mb-2">{label}</p>
      {payload.map((entry, i) => (
        <div key={i} className="flex items-center gap-2 text-sm">
          <div className="w-2 h-2 rounded-full" style={{ backgroundColor: entry.color }} />
          <span className="text-white/60">{entry.name}:</span>
          <span className="text-white font-medium">{formatter ? formatter(entry.value, entry.name) : entry.value}</span>
        </div>
      ))}
    </div>
  )
...
export function Badge({ children, variant = 'default' }) {
  ...
  return <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-[11px] font-medium border ${variants[variant]}`}>{children}</span>
}

export function ProgressBar({ value, max, color = 'cyan', showLabel = true, size = 'md' }) {
  const pct = max > 0 ? Math.min((value / max) * 100, 100) : 0
  const colors = { cyan: 'from-cyan-500 to-cyan-400', emerald: 'from-emerald-500 to-emerald-400', violet: 'from-violet-500 to-violet-400', amber: 'from-amber-500 to-amber-400', rose: 'from-rose-500 to-rose-400' }
  ...
  return (
    <div className="w-full">
      ...
      {showLabel && <p className="text-[10px] text-white/30 mt-1 text-right">{pct.toFixed(0)}%</p>}
    </div>
  )
}
