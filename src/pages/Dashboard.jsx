...
function TabVisaoGeral({ data, metrics }) {
  ...
  // Cor da meta: baseada no pace proporcional ao dia atual do mes
  // pace esperado = (diaAtual / diasNoMes) * metaValor
  const pctMeta = metrics.atingimentoValor
  const paceEsperado = metrics.diasNoMes > 0 ? (metrics.diaAtual / metrics.diasNoMes) * metrics.metaValor : 0
  const noMeta = metrics.totalWonValor >= paceEsperado
  const metaColor = noMeta ? 'emerald' : 'rose'
  const metaColorHex = noMeta ? '#10b981' : '#ef4444'
  const metaColorText = noMeta ? 'text-emerald-400' : 'text-rose-400'
  const metaColorBg = noMeta ? 'from-emerald-500/10 to-emerald-500/5' : 'from-rose-500/10 to-rose-500/5'
  const metaColorBorder = noMeta ? 'border-emerald-500/20' : 'border-rose-500/20'
  const metaStatusLabel = pctMeta >= 100 ? 'Meta batida!' : noMeta ? 'No pace' : 'Atencao'
